...
pub enum TapretProofError {
    ...
    /// Errors in the taproot script tree and tapret path proof correspondences.
    /// See [`TapretSourceError`] for details.
    #[from]
    #[display(inner)]
...
pub enum TapretSourceError {
    ...
    /// unable to produce tapret commitment since the commitment path {0} does not
